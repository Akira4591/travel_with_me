...
当前项目处于 **S0 本地 MVP → S1 工程可私测** 的过渡期。

最近一次验证：2026-06-19。

| 检查项                 | 结果     | 说明                                                     |
| ---------------------- | -------- | -------------------------------------------------------- |
| `npm.cmd test`         | 通过     | 5 个测试文件，43 个测试通过                              |
| `npm.cmd run check`    | 通过     | Prettier 与 ESLint 均通过                                |
| `npm.cmd run test:e2e` | 通过     | Playwright 桌面 + 移动端 shell smoke 通过                |
| Git 工作区             | 脏工作区 | 存在大量历史修改和新增文件，后续改动需避免误回滚用户内容 |
...
- 数据导出/导入、移动端核心体验和浏览器级业务路径回归测试尚未闭环。
- localStorage 仍是主存储，旧 schema 会丢弃。
- 移动端未系统适配。
- 浏览器级测试已有 shell smoke，但尚未覆盖添加地点、编辑行程、分享图和 3D。
...
- 小屏采用列表/地图切换或底部抽屉。
- 弹窗高度、搜索结果、导入预览、分享预览可滚动且不遮挡主操作。
- 拖拽排序在移动端至少有替代操作。

验收标准：

- 375px 宽度下可以完成创建路线、添加地点、切换日期、生成分享图。

### 6. 核心路径浏览器测试（基础 smoke 已完成，业务路径待扩展）

- 增加 Playwright 或等价工具的 smoke tests。
- 覆盖启动、创建行程、添加地点、编辑事件、切换 day、生成分享图。
- 3D 如果启用，至少验证 canvas 非空、切换按钮状态正确。
