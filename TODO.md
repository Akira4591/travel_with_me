...
当前项目处于 **S1 工程可私测已闭环 → S2 差异化验证收口** 的阶段。

最近一次验证：2026-06-19。

| 检查项                          | 结果       | 说明                                                                                                     |
| ------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------- |
| `npm.cmd test`                  | 通过       | 7 个测试文件，51 个测试通过                                                                              |
| `npm.cmd run test:guide-import` | 通过       | 12 个 AI 导入种子样例，64 个标注地点；召回、day、note、类型识别均通过当前阈值                            |
...
### 4. 3D Diorama 价值验证（S2-5 已完成标记模型首批）
